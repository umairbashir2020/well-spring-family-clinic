...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (heightUnit) {
        heightUnit.addEventListener('change', function() {
            updateHeightPlaceholder(heightInput, this.value);
        });
        // Initialize placeholder on page load
        updateHeightPlaceholder(heightInput, heightUnit.value);
    }
});

function updateHeightPlaceholder(heightInput, unit) {
    if (unit === 'cm') {
        heightInput.placeholder = 'e.g., 175';
    } else {
        heightInput.placeholder = 'e.g., 1.75';
...
function displayResults(bmi, category) {
    ...
    // Switch from placeholder to results
...
// SIMPLIFIED input validation - Only prevent negative numbers
document.addEventListener('DOMContentLoaded', function() {
    const numberInputs = document.querySelectorAll('input[type="number"]');
    
    numberInputs.forEach(input => {
        input.addEventListener('input', function() {
            // Only prevent negative values, don't modify the input otherwise
            if (this.value < 0) {
                this.value = '';
            }
        });
